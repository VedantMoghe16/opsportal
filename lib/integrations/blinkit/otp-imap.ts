...
/**
 * Poll the OTP inbox over IMAP (Gmail app password) for the partnersbiz OTP and
 * return the code. Searches messages since `sentAfter`, newest first.
 */
export async function waitForOtpImap(opts: {
  ...
  const user = env.BLINKIT_OTP_EMAIL || env.BLINKIT_LOGIN_EMAIL;
  const pass = env.BLINKIT_OTP_APP_PASSWORD;
  if (!user || !pass) {
    throw new Error("[blinkit] OTP IMAP not configured: set BLINKIT_LOGIN_EMAIL + BLINKIT_OTP_APP_PASSWORD");
  }
    ...
    try {
      while (Date.now() < deadline) {
        ...
        const uids = await client.search(
          { since: new Date(floor), or: [{ from: "partnersbiz" }, { subject: "otp" }, { subject: "verification" }, { subject: "code" }] },
          { uid: true },
        );
        ...
        for (const uid of candidates) {
          ...
          console.log(
            `[blinkit:otp] candidate from="${from}" subject="${subject.slice(0, 80)}" ` +
  ...
  throw new OtpNotFoundError("No partnersbiz OTP email found within the wait window");
}
